...
export function ProfileModal({ person, onClose }: ProfileModalProps) {
  ...
  const displayCitation = isLongCitation && !isExpanded 
    ? person.citation.substring(0, 200) + '...' 
    : person.citation;

  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-sm modal-backdrop-enter overflow-y-auto pt-2 md:pt-4 pb-2" onClick={onClose}>
      <div
        className="bg-card/95 backdrop-blur-md gold-border rounded-xl max-w-4xl w-full mx-3 md:mx-4 overflow-hidden shadow-[0_10px_50px_rgba(212,175,55,0.15)] modal-enter relative mt-0 md:mt-1"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-primary/10 bg-muted/20">
          ...
          <button onClick={onClose} className="p-1.5 rounded-md hover:bg-destructive/10 transition-colors text-muted-foreground hover:text-destructive active:scale-95 bg-background border border-border">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="p-5 md:p-6">
          <div className="flex flex-col sm:flex-row gap-5 mb-6">
            <div className="h-28 w-28 sm:h-36 sm:w-36 rounded-xl bg-muted/40 flex items-center justify-center text-primary border-2 border-primary/30 shrink-0 self-center sm:self-start shadow-inner relative overflow-hidden group">
              ...
              {resolvedImageUrl ? (
                <img src={resolvedImageUrl} alt={person.name} className="h-full w-full object-cover rounded-lg relative z-10" />
              ) : (
            ...
            >
              ...
              <h4 className="text-2xl md:text-3xl font-bold font-serif text-foreground mb-2 leading-tight">{person.name}</h4>
              <div className="flex items-center gap-3">
                <p className="text-sm font-medium px-3 py-1 rounded-md bg-muted text-muted-foreground border border-border">{person.service}</p>
                <div className="h-1 w-1 rounded-full bg-primary/50" />
                <p className="text-sm text-muted-foreground font-semibold">{person.periodStart} – {person.periodEnd}</p>
              </div>
            </div>
          </div>
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-widest text-primary/70 mb-1">Service Category</p>
            <p className="text-base font-medium text-foreground">{person.category}</p>
          </div>
          ...
          >
             <div className="flex items-center gap-2 mb-3">
               ...
               <p className="text-xs uppercase tracking-widest font-bold text-muted-foreground">Official Bio / Citation</p>
             </div>
