...
const config = fs.readFileSync('src/components/AdminPanel.tsx', 'utf-8');
const start = "{tab === 'theme' && (";
const end = "{tab === 'transitions' && (";
const idx1 = config.indexOf(start);
const idx2 = config.indexOf(end);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr_280px] gap-3 h-[60vh] min-h-[400px]">
              ...
              <div className="rounded-lg border border-primary/20 bg-slate-900/60 flex flex-col overflow-hidden shadow-inner relative">
                <div className="p-2 border-b border-primary/10 flex justify-between items-center absolute top-0 w-full z-10 bg-gradient-to-b from-slate-950/90 to-transparent">
                     <span className="text-[10px] uppercase tracking-widest text-slate-400 font-medium dropshadow-sm">Grade Canvas Preview</span>
                </div>
              ...
              <div className="rounded-lg border border-primary/20 bg-slate-900/60 flex flex-col overflow-hidden shadow-inner">
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-4 custom-scrollbar">
                  <div className="bg-slate-950/60 rounded-md border border-primary/10 p-3 flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-0.5 custom-scrollbar border border-slate-800 rounded bg-slate-950/80 p-0.5 max-h-48">
...
const result = config.substring(0, idx1) + replacement + config.substring(idx2);
fs.writeFileSync('src/components/AdminPanel.tsx', result);
console.log('patched');
